...
const makeExchangeInfo = (
  ...
  reserves: [
    {
      reserveAccount: new PublicKey('Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB'),
      adminFeeAccount: new PublicKey('Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB'),
...
const assertTokenAmounts = (actual: TokenAmount, expected: TokenAmount) => {
  expect(actual.equalTo(expected) && actual.token.equals(expected.token)).toBe(true)
}
...
describe('Calculated amounts', () => {
  ...
  describe('#calculateEstimatedMintAmount', () => {
    ...
    it('fees if unequal liquidity provision', () => {
      ...
      assertTokenAmount(result.fees, JSBI.subtract(result.mintAmountBeforeFees.raw, expectedMintAmount))
    })
  })
